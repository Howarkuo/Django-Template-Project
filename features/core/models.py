...
class User(AbstractUser):
    ...
    id = models.UUIDField(verbose_name="userId", primary_key=True, unique=True, default=uuid.uuid4, auto_created=True)
    ...
    def delete(self) -> None:
        ...
        # self.deleted_at = datetime.now(tz=settings.TIME_ZONE)
...
class ModelConfig:
    """ModelConfig class for generate indexes and constraints for a model."""

    @staticmethod
    def generate_model_indexes(model_name: str, code_column: str = "code") -> list[models.Index]:
        """Generate indexes for a model."""
        return [
            models.Index(fields=["company_id"], name=f"{model_name}_company_id_idx"),
            models.Index(
                fields=[code_column, "company_id"],
                name=f"{model_name}_{code_column}_company_id_idx",
                condition=models.Q(is_delete=False),
            ),
        ]

    @staticmethod
    def generate_model_constraints(model_name: str, code_column: str = "code") -> list[models.UniqueConstraint]:
        """Generate constraints for a model."""
        return [
            models.UniqueConstraint(
                fields=[code_column, "company_id"],
                name=f"{model_name}_{code_column}_company_id_unique",
                condition=models.Q(is_delete=False),
            ),
        ]
